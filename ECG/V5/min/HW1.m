...
% half = val(t) ;
% detrend = detrend(half) ;
% [pks,locs] = findpeaks(detrend,'MinPeakHeight',100);
% tv = t(locs);
% RR = diff(tv);
% RRmed = median(RR) ;
% 
...
EKG1 = detrend;
EKG2 = detrend;
[R1,TR1]  = findpeaks( EKG1, t, 'MinPeakHeight',100);
[Q1,TQ1]  = findpeaks(-EKG1, t, 'MinPeakHeight',50);              % NOTE: No 0S’ Waves In EKG1
[R2,TR2]   = findpeaks( EKG2, t, 'MinPeakHeight', 100);
[QS2,TQS2] = findpeaks(-EKG2, t, 'MinPeakHeight', 40);
figure(1)
subplot(2,1,1)
plot(t, EKG1)
hold on
